...
import { MODES, BRAWLERS } from "@/data/gameData";
    ...
    <div className="grid gap-4" style={{ gridTemplateColumns: "1fr 1fr", gridTemplateRows: "auto" }}>
      ...
      <div className="space-y-4">
        ...
        <div className="relative overflow-hidden rounded-2xl p-5"
          ...
          <div className="absolute inset-0"
            style={{ background: "radial-gradient(ellipse at 80% 50%, rgba(156,39,176,0.3), transparent 60%)" }} />
          <div className="absolute top-3 right-5 text-6xl opacity-20 animate-float">🌟</div>
          <div className="relative">
            ...
            <h3 className="font-bebas text-3xl tracking-wide text-white mb-0.5">Пески Времени</h3>
        ...
        <button
          ...
          className="w-full rounded-2xl py-5 font-bebas text-3xl tracking-widest relative overflow-hidden transition-all duration-200 hover:scale-[1.02] active:scale-[0.97]"
          style={{
            background: "linear-gradient(135deg, #FF6B00, #FFB800)",
            color: "#0A0B14",
            boxShadow: "0 0 40px rgba(255,184,0,0.45), 0 4px 24px rgba(0,0,0,0.5)",
          }}>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {BRAWLERS.slice(0, 4).map((b) => (
              <div key={b.id} className="flex-1 rounded-xl overflow-hidden transition-all hover:scale-105 cursor-pointer"
                style={{ background: "#1A1D2E", border: `1px solid ${b.rarityColor}40` }}>
                <img src={b.image} alt={b.name} className="w-full h-16 object-cover" />
        ...
        <div className="rounded-2xl p-4 relative overflow-hidden"
          ...
          <div className="relative">
            <div className="font-oswald font-bold text-base mb-1.5" style={{ color: "#FFB800" }}>
              Система Баффов
            </div>
            <p className="text-xs opacity-60 mb-3">Три усиления для каждого бойца: гаджет, гиперзаряд и звёздная сила</p>
            <button className="w-full py-2 rounded-xl text-xs font-bold transition-all hover:scale-105"
